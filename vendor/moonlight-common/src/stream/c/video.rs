...
#[doc(hidden)]
pub fn presentation_timestamp_from_raw_micros(presentation_time_us: u64) -> Duration {
...
unsafe fn convert_decode_unit<'a>(decode_unit: PDECODE_UNIT) -> VideoDecodeUnit<&'a [u8]> {
    ...
    VideoDecodeUnit {
        ...
        timestamp: presentation_timestamp_from_raw_micros(raw.presentationTimeUs),
...
mod tests {
    ...
    use moonlight_common_sys::limelight::_DECODE_UNIT;

    use super::convert_decode_unit;

    fn convert_raw_timestamp(presentation_time_us: u64) -> Duration {
        // A zeroed decode unit is valid for this focused conversion test:
        // frame type and color space both use zero as a valid value, and a
        // null buffer list represents an empty frame.
        let mut raw = unsafe { MaybeUninit::<_DECODE_UNIT>::zeroed().assume_init() };
        raw.presentationTimeUs = presentation_time_us;

        unsafe { convert_decode_unit(&mut raw) }.timestamp
    }

    #[test]
    fn preserves_raw_presentation_time_across_old_overflow_boundary() {
        ...
        for presentation_time_us in [
            ...
            assert_eq!(
                convert_raw_timestamp(presentation_time_us),
                Duration::from_micros(presentation_time_us)
            );
        }
    }
